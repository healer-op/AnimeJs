...
if(label){
fetch(`https://api.consumet.org/movies/flixhq/${label}`)
.then(response =>{
    if(!response.ok){
        throw Error("ERROR");
    }
        ...
        <div class="col-lg-4 col-md-6 col-sm-6">
                                <div class="product__item">
                                    ...
                                    <div class="product__item__text">
                                        <h5><a href="netflix.html?view=${data.results[i].id}">${data.results[i].title}</a></h5>
                                    </div>
                                </div>
                            </div>`
    }).join('');
...
if(keyword){
    fetch(`https://api.consumet.org/movies/flixhq/${keyword}`)
    .then(response =>{
        if(!response.ok){
            throw Error("ERROR");
        }
